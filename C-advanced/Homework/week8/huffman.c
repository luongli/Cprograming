...
*/
JRB dequeue_Prior(JRB priorQ, JRB *ptr, int count)
{
	JRB p;
...
HuffmanTree makeHuffman(char *buffer, int size)
{
	...
	while(q_pointer != jrb_nil(priorQ)){
		...
		*/
		count++;
		node1 = dequeue_Prior(priorQ, &q_pointer, count);
		count++;
		node2 = dequeue_Prior(priorQ, &q_pointer, count);
		...
		if(node1 != jrb_nil(priorQ) && node2 != jrb_nil(priorQ)){
			...
			// get parent id
			parent_id = ch1 + ch2;
			insertEdge(g, parent_id, ch1, 0);
			insertEdge(g, parent_id, ch2, 1);
			addVertex(g, parent_id, NULL);
			addVertex(g, ch1, NULL);
			addVertex(g, ch2, NULL);

			// add parent to priorQ
			jrb_insert_int(priorQ, freq1+freq2, ji(parent_id));
		}
...
void addHuffmanChar(int ch, Coding *htable, int *bits, int n)
{
	...
	/* convert a array of bits into bits in an integer */
	for(i = 0; i < n; i++){
		printf("%d", bits[i]);
		code = code | (bits[i] << i);
	}
...
void createHuffmanTable(HuffmanTree htree, int root, Coding *htable, int level, int *bits)
{
	...
	if(!g.edges && !g.vertices){
		/* this means buffer contains 1 character
		set that character code to 0
		*/
	...
	int n = getAdjVertices(g, root, adjVertices);
	...
	for(i = 0; i < 2; i++){
		bits[level] = (int) getWeight(g, root, adjVertices[i]);
